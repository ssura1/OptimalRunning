...
| App bundle layout | single-target app (`WKApplication`) **plus `WKWatchOnly`**. Two keys, two questions: layout, and whether a companion iPhone app exists. Setting only the first installs fine on the Simulator and is refused by a real watch with `MIInstaller` error 97 — see [T-099](./implementation.md#t-099). | **two-target** WatchKit app + extension (`WKWatchKitApp`). `WKApplication` is watchOS 9+, so a single-target bundle will not launch on Series 3. |
