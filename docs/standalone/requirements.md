...
| NFR-S-8 (step count ±2%) | ±2% | **First human count, 2026-08-09.** Over the 60.0 s of `run-2026-08-09-2326-tempo` the runner counted **161**. Independently: spectral integral **162.3**, an impact detector sharing no code with the estimator **162–163**, `CMPedometer` **156** (−3.9%). The shipped detector read **182** before the back-fill fix below and **158** after — **−1.9% against the count**, inside the bound. On the pace ladder the detector reads 2204 against **2200** from the spectral integral (**+0.2%**), unchanged by the fix | **Measured for the first time, and inside ±2% — but not validated.** Three reasons, all recorded rather than waved past: the segment is **60 s against a requirement that says ≥ 5 minutes**; it is **one count by one runner**, with its own unstated error; and the trace format cannot carry it, so it lives in prose rather than in a `references` block ([Fixtures/motion/README.md](../../Fixtures/motion/README.md)) |
| — (steps while stationary) | 0 | A phone left still for 20.5 minutes on `capture-2026-08-09-1924` produced **1526 steps** at a sustained 75.3 spm. The stationarity gate was firing correctly for 97.3% of samples; it suppressed events **without advancing the phase anchor**, so the phase-locked fallback back-filled the whole stop the instant a table knock reopened the gate. Now **52** | **Fixed and pinned.** `StationaryBackfillTests` fails on the pre-fix detector and passes after; the marked running segment is bit-identical at 153 steps across the fix, which is what distinguishes a fix from a suppression. The pace-ladder trace is bit-identical too, so [S-064](./implementation.md#s-064)'s reasoning is untouched |
